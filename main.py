...
print("IM ON MAIN.PY FILE")
load_project_path = os.path.abspath(os.path.dirname(__file__))
print("load_project_path: ", load_project_path)
...
parser.add_argument('--load_anno_csv_path', type=str, default='D:/BAI/FacialMMT-main/MELD')
parser.add_argument('--meld_text_path', type=str, default='D:/BAI/FacialMMT-main/pretrained_model')
...
parser.add_argument('--trg_batch_size', type=int, default=2, help='num of dialogues in MELD')    
...
print("after the parser")
print(args)

torch.manual_seed(args.seed)
if torch.cuda.is_available():
    print("Im running stuf on gpu :) ")
    torch.cuda.manual_seed(args.seed)
    torch.set_default_tensor_type('torch.FloatTensor')

if args.choice_modality == 'V':
    print("Im running only Vision")
    ...
    print("IM RUNNING TEXT AUDIO VISION")
    ...
    print("after getting train data")
    trg_valid_data = get_multimodal_data(args, 'val')
    print("after getting valid data")
    trg_test_data = get_multimodal_data(args, 'test')
    print("after getting test data")
    if not args.doEval:
        #auxiliary dataset
...
print("after loading everything")

if args.choice_modality == 'T+A+V':
    print("Im here: args.choice_modality == 'T+A+V' ")
    args.audio_featExtr_dim = trg_train_data.get_audio_featExtr_dim()  
    print("after extracting audio")

if args.choice_modality in ('V', 'T+A+V'):
    print("Im here: if args.choice_modality in ('V', 'T+A+V')")
    args.vision_featExtr_dim = trg_train_data.get_vision_featExtr_dim()
    print("after extracting vision")

if args.choice_modality == 'T+A+V':
    print("IM HERE: if args.choice_modality == 'T+A+V':")
...
if __name__ == '__main__':
    ...
    if args.doEval:
        ...
        if args.choice_modality == 'V':
            ...
            print("elif args.choice_modality == 'T+A+V':")
