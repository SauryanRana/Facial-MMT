...
class Lite(LightningLite):
    def run(self, args, aux_train_loader, trg_train_loader, trg_valid_loader, trg_test_loader):
        ...
        def aux_train(self, shareSwin_model, share_model_optimizer, share_model_scheduler, criterion):
            print("IM INSIDE aux_train function")
            print("shareSwin_model: ", shareSwin_model)
            print("share_model_optimizer: ", share_model_optimizer)
            print("share_model_scheduler: ", share_model_scheduler)
            ...
            print("after share model optimizer")
            
            print("IM GETTINT INSIDE THIS LOOP: for i_batch, (image_feature, labels) in enumerate(aux_train_loader): ")
        ...
        def multimodal_train(self, shareSwin_model, multimodal_model, multimodal_model_optimizer, multimodal_model_scheduler, criterion):
            print("IM HERE: LITE function: multimodal_train")
            shareSwin_model.train()
        ...
        def multimodal_evaluate(shareSwin_model, multimodal_model, criterion, test=False):
            print("IM INSIDE multimodal evaluate fucniton")
            #print("shareSwin_model: ", shareSwin_model)
            ...
            with torch.no_grad():
                ...
                for i_batch, batch in enumerate(loader):    
                    ...
                    for utt_idx in range(len(vision_inputs)):
                        print("utt_idx: ", utt_idx)
                        curr_utt_num_imgs = batch_vis_num_imgs[utt_idx]
                        print("curr_utt_num_imgs: ", curr_utt_num_imgs)
                        curr_utt_img_feat = batch_vision_utt_feat_aux_task[utt_idx][:curr_utt_num_imgs]
                        batch_img_feat_list.append(curr_utt_img_feat)
                        utt_idx += 1
        ...
        if not args.doEval:
            ...
            for epoch in range(1, args.num_epochs+1): 
                if args.choice_modality == 'T+A+V':
                    print("if args.choice_modality == 'T+A+V':")
                    #----------------------------------------------------auxiliary task---------------------------------------------------------#
                    start = time.time()
                    ...
                    print("im on line 373 : train.py")
                    val_wg_av_f1 = eval_meld(results, truths, test=False)
                    print("val_wg_av_f1: ", val_wg_av_f1)
                    ...
                    if val_wg_av_f1 > best_valid_f1:
                        print("INSIDE IF val_wg_av_f1 > best_valid_f1")
        ...
        #conduct evaluation directly without training
            ...
            print("we are conducting evaluation directly without training")
            ...
            if args.choice_modality == 'T+A+V' :
                ...
                print("elif args.choice_modality == 'V':")
